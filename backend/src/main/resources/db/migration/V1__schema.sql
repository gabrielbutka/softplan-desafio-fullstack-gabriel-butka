...
create table usuario (
    id bigserial not null constraint usuario_pk primary key,
...
create table processo (
    id bigserial not null constraint processo_pk primary key,
    usuario_id bigint not null constraint processo_fk_usuario references usuario,
...
create table processo_interessado (
    processo_id bigint not null constraint processo_interessado_fk_processo references processo,
    usuario_id  bigint not null constraint processo_interessado_fk_usuario references usuario,
...
create table parecer (
    id bigserial not null constraint parecer_pk primary key,
    processo_id bigint not null constraint parecer_fk_processo references processo,
    usuario_id  bigint not null constraint parecer_fk_usuario references usuario,
    descricao varchar(1000) not null,
    criacao timestamp not null,
    edicao timestamp not null,
